...
namespace GXPEngine.Core
{
    public struct Vector2 : IEquatable<Vector2>, IFormattable
    {
        ...
        public static float AngleBetween2(Vector2 a, Vector2 b)
        {
            ...
            return (float)System.Math.Acos(d) * Mathf.Rad2Deg;
        }
        ...
        {
            var rd = useRadians ? Mathf.PI : 180f;
            var ra = Wrap(a2 - a1, rd * 2f);
            if (ra > rd) ra -= (rd * 2f);
            return ra;
        }

        public static float ShortenAngfloatoAnother(float dep, float ind, bool useRadians)
        {
        ...
        public static Vector2 RotateToward(Vector2 from, Vector2 to, float a, bool bUseRadians = false)
        {
            ...
            a2 = ShortenAngfloatoAnother(a2, a1, true);
        ...
        public static float AngleBetween(Vector2 a, Vector2 b)
        {
            ...
            if (d < Mathf.DBL_EPSILON) return 0f;

            d = (double)Vector2.Dot(a, b) / d;
            if (d >= 1d) return 0f;
            else if (d <= -1d) return 180f;
            return (float)System.Math.Acos(d) * Mathf.Rad2Deg;
        }

        public static float CalculateLookDirection(Vector2 a, Vector2 b)
        {
            Console.WriteLine(a);
            Console.WriteLine(b);
            float dx = a.x - b.x;
        ...
        public static double bearing(Vector2 a, Vector2 b)
        {
            const double TWOPI = 6.2831853071795865;
            const double RAD2DEG = 57.2957795130823209;
            // if (a1 = b1 and a2 = b2) throw an error 
            double theta = Mathf.Atan2(b.x - a.x, a.y - b.y);
            if (theta < 0.0)
                theta += TWOPI;
        ...
        {
            float angle = (float)Math.Atan2(b.y * a.x - b.x * a.y, b.x * a.x + b.y * a.y);
            return angle;
        }


        public static float FromAToB(Vector2 A, Vector2 B)
        {
            return Mathf.Atan2(B.y, B.x) * Mathf.Rad2Deg;
        }

        public static float getAngleDegrees(Vector2 from, Vector2 to, bool force360 = true)
        {
            float deltaX = from.x - to.x;
            float deltaY = from.y - to.y; // reverse
            float radians = (float)Math.Atan2(deltaY, deltaX);
            float degrees = (radians * 180) / (float)Math.PI - 90; // rotate
            if (force360)
            {
        ...
        //public static Vector2 SmoothDamp(Vector2 current, Vector2 target, ref Vector2 currentVelocity, float smoothTime, float maxSpeed = float.PositiveInfinity, [DefaultValue("Time.deltaTime")] float deltaTime)
        //{
